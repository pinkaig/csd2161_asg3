...
void ErrMsg(const std::string& funcname) {
    std::cerr << funcname << " failed. WSA_EC(" << WSAGetLastError() << ")" << std::endl;
}
    ...
    std::thread send_thread([&]() {
        ...
        while (running) {
            ...
            if (usermsg == "/q")
            {
                ...
                send(serverSocket, reinterpret_cast<const char*>(&quitCommand), sizeof(quitCommand), 0);

                // Now close the socket and cleanup
                shutdown(serverSocket, REQ_QUIT); // Optional but gracefully shuts down sending
                closesocket(serverSocket);
                WSACleanup();
                ...
                std::string command_less = usermsg, sendmsg{ 0 }, temp{};
                command_less.erase(0, 2);
                uint32_t text_networkLength = htonl(uint32_t(command_less.length()));
                std::stringstream ss(command_less), packed{};
                while (ss >> std::setw(2) >> temp) {
                    int hexValue = std::stoi(temp, 0, 16);
                    packed << char(hexValue);
                }
                sendmsg = packed.str();
                bytesSent = send(serverSocket, sendmsg.c_str(), static_cast<uint32_t>(sendmsg.length()), 0);
                ...
                int bytesSent = send(serverSocket, reinterpret_cast<const char*>(&listUsersCommand), sizeof(listUsersCommand), 0);
                if (bytesSent == SOCKET_ERROR) {

                    break;
                }
            }
            else if (usermsg.substr(0, 2) == "/e") {
                // Handle echo command...
                std::string ip;
                uint16_t port;
                std::string message;
                if (usermsg.size() > 3 && usermsg[2] == ' ') {
                    if (parseEchoCommand(usermsg, ip, port, message)) {
                       ...
                       // std::cout << "Sending to " << ip << ":" << port << " with message: " << message << std::endl;
                        ...
                        // Send the packet
                        send(serverSocket, reinterpret_cast<const char*>(packet.data()), packet.size(), 0);
                    ...
                    shutdown(serverSocket, SD_SEND); // Optional but gracefully shuts down sending
                    closesocket(serverSocket);
                    WSACleanup();
                    //std::cerr << "disconnecting..." << std::endl;
                    running = false;
