...
\date  2024-03-16
\brief This program implements a multi-threaded TCP and UDP server using Winsock.
...
int sendFile(FILE* fp, SOCKET udpSocket, sockaddr_in clientAddr, int clientAddrLen) {
	char buffer[NET_BUF_SIZE];

	while (true) {
		int bytesRead = fread(buffer, 1, NET_BUF_SIZE, fp);
		if (bytesRead <= 0) {
			break; // End of file
		}

		int bytesSent = sendto(udpSocket, buffer, bytesRead, sendrecvflag,
	...
	// Send an explicit "END_OF_FILE" marker
	const char* endSignal = "END_OF_FILE";
	sendto(udpSocket, endSignal, strlen(endSignal), sendrecvflag,
		(struct sockaddr*)&clientAddr, clientAddrLen);
...
int main(int argc, char* argv[])
{
	...
	serverAddr.sin_port = htons(port); // Use appropriate UDP port
	...
	std::cout << "UDP server port number: " << port << "\n";

	if (argc >= 2) {
		// Handle case when input file is provided
	}
	else if (argc < 2) {
		if (argc == 1)
			std::cout << "TCP Server Port Number: ";
		while (!(std::cin >> port)) {
...
bool execute(SOCKET clientSocket)
{
	...
	while (true)
	{
		...
		if (commandId == REQ_LISTFILES) {
			...
			std::vector<unsigned char> response;
			response.push_back(RSP_DOWNLOAD);  // Response ID
			sockaddr_in clientAddr;
			int clientAddrLen = sizeof(clientAddr);
			char net_buf[NET_BUF_SIZE];
			clearBuf(net_buf); // Clear the buffer before receiving data
			...
			std::string fileName(net_buf + 1, nBytes - 1); // Extract filename
			std::cout << "Requested File: " << fileName << std::endl;

			// Open the file and send it via UDP
			FILE* fp;
			errno_t err = fopen_s(&fp, fileName.c_str(), "rb");
			if (err != 0 || fp == NULL) {
				std::cerr << "File open failed!" << std::endl;
				strcpy_s(net_buf, NET_BUF_SIZE, "File Not Found");
				sendto(udpSocket, net_buf, strlen(net_buf), sendrecvflag,
					(struct sockaddr*)&clientAddr, clientAddrLen);
			}
			else {
				std::cout << "File successfully opened! Sending file...\n";
				sendFile(fp, udpSocket, clientAddr, clientAddrLen);
				fclose(fp);
			}
		}
